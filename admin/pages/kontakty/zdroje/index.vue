...
const pageTitle = ref('Zdroje kontaktů');
...
async function loadItems() {
	...
	await client<{ id: number }>('/api/admin/contact/source', {
		...
		toast.add({
			title: 'Chyba',
			description: 'Nepodařilo se načíst zdroje kontaktů. Zkuste to prosím později.',
			color: 'red',
		});
	}).finally(() => {
		loading.value = false;
...
async function deleteItem(id: number) {
	...
	await client<{ id: number }>('/api/admin/contact/source/' + id, {
		method: 'DELETE',
		headers: {
			'Accept': 'application/json',
			'Content-Type': 'application/json',
		...
		toast.add({
			title: 'Chyba',
			description: 'Nepodařilo se smazat položku zdroje.',
			color: 'red',
		});
	}).finally(() => {
		loading.value = false;
		...
		<LayoutContainer>
			<BaseTable
				...
				:columns="[
					...
					{ key: 'name', name: 'Název', type: 'badge', width: 80, hidden: false, sortable: true, colorKey: 'color' },
