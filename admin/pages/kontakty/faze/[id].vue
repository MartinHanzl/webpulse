...
async function saveItem() {
	...
	await client<{
		...
		name: string;
		color: string;
	}>(route.params.id === 'pridat' ? '/api/admin/contact/phase' : '/api/admin/contact/phase/' + route.params.id, {
		method: 'POST',
		...
		toast.add({
			title: 'Chyba',
			description: 'Nepodařilo se uložit fázi procesu. Zkontrolujte, že máte vyplněna všechna pole správně a zkuste to znovu.',
			color: 'red',
		});
	}).finally(() => {
		loading.value = false;
