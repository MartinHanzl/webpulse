...
async function loadItem() {
	...
	await client<{
		...
		breadcrumbs.value.push({
			...
			link: '/nastavnei/jazyky/' + route.params.id,
			current: true,
		});
...
async function saveItem() {
	...
	await client<{
		...
		toast.add({
			title: 'Hotovo',
			description: route.params.id === 'pridat' ? 'Jazyk byl úspěšně vytvořen.' : 'Jazyk byl úspěšně upraven.',
			color: 'green',
		});
		...
		toast.add({
			title: 'Chyba',
			description: 'Nepodařilo se upravit jazyk. Zkontrolujte, že máte vyplněna všechna pole správně a zkuste to znovu.',
			color: 'red',
		});
	}).finally(() => {
		loading.value = false;
...
<template>
	<div>
		<LayoutHeader
			:title="route.params.id === 'pridat' ? 'Nový jazyk' : item.name"
