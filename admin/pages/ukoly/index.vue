...
async function loadItems() {
	...
	await client<{ id: number }>('/api/admin/contact/task', {
		...
		toast.add({
			title: 'Chyba',
			description: 'Nepodařilo se načíst úkoly. Zkuste to prosím později.',
			color: 'red',
		});
	}).finally(() => {
		loading.value = false;
...
async function deleteItem(id: number) {
	...
	await client<{ id: number }>('/api/admin/contact/task/' + id, {
		method: 'DELETE',
		headers: {
			'Accept': 'application/json',
			'Content-Type': 'application/json',
		...
		toast.add({
			title: 'Chyba',
			description: 'Nepodařilo se smazat položku úkolu.',
			color: 'red',
		});
	}).finally(() => {
		loading.value = false;
