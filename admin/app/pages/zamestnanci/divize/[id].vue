...
async function loadItem() {
  ...
  await client('/api/admin/employee/division/' + route.params.id, {
    ...
    .then((r) => {
      ...
      breadcrumbs.value[2] = {
        name: r.name,
        link: '/zamestnanci/divize/' + route.params.id,
        current: true,
      };
    })
    .catch(() => {
...
async function saveItem(redirect = true) {
  ...
  await client(
    route.params.id === 'pridat'
      ? '/api/admin/employee/division'
      : '/api/admin/employee/division/' + route.params.id,
    {
      method: 'POST',
    ...
    .then((r) => {
      $toast.show({
        summary: 'Hotovo',
        detail: route.params.id === 'pridat' ? 'Divize vytvořena.' : 'Divize upravena.',
        severity: 'success',
      });
    ...
    .catch(() => {
      $toast.show({ summary: 'Chyba', detail: 'Nepodařilo se uložit divizi.', severity: 'error' });
    })
    .finally(() => {
      loading.value = false;
    });
