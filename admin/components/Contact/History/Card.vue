			...
			<div class="px-4 py-5 sm:px-6">
				<div class="grid grid-cols-3">
					<div class="col-span-1">
						<time class="mb-1 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">{{ new Date(history.created_at).toLocaleString() }}</time>
						<div class="flex items-end">
							...
							<h3 class="text-lg font-semibold text-grayDark ml-4">
								{{ history.name }}
							</h3>
						</div>
					</div>
					...
					<div class="col-span-1 text-end flex items-end justify-end">
						<PencilIcon
							v-if="history.origin === 'user'"
							class="size-5 shrink-0 text-primaryCustom hover:text-primaryLight cursor-pointer"
							@click="emit('edit-history', history)"
						/>
						<TrashIcon
							v-if="history.origin === 'user'"
							class="ml-4 size-5 shrink-0 text-danger hover:text-dangerLight cursor-pointer"
