	...
	<main class="mx-auto flex min-h-[80vh] w-full max-w-xl flex-col items-center gap-8 py-6">
			...
			<div class="flex flex-col items-center gap-3">
				...
				<div class="inline-flex rounded-full border border-gray-300 bg-white p-1 shadow-sm">
					<button
						...
						:class="mode === 'word' ? 'bg-primary text-white shadow' : 'text-gray-600 hover:bg-gray-100'"
						@click="mode = 'word'"
					>
						Tajné slovo
					</button>
					<button
						class="rounded-full px-5 py-2 text-sm font-semibold transition"
						:class="mode === 'question' ? 'bg-primary text-white shadow' : 'text-gray-600 hover:bg-gray-100'"
						@click="mode = 'question'"
					>
						Otázka
					</button>
		...
		<section v-else-if="phase === 'reveal'" class="flex w-full flex-col items-center gap-6">
			...
			<div
				...
				:class="current.isImposter ? 'border-rose-300 bg-rose-50' : 'border-primary/40 bg-primary/5'"
			>
		...
		<section v-else-if="phase === 'discuss'" class="flex w-full flex-col items-center gap-6">
			...
			>
				...
				<p class="max-w-sm text-sm text-gray-500">
					...
					<template v-else>
						Každý nahlas odpoví na svou otázku. Pak se domluvte, čí odpověď neseděla – to je imposter.
					</template>
				</p>
