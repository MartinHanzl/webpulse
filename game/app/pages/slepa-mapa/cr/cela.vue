...
const addMapBoundaries = async () => {
  ...
  try {
    const sources = [
      {
        url: 'https://raw.githubusercontent.com/smartmaps/districts-cz/master/districts-cz.json',
        style: borderStyles.districts,
      },
      {
        url: 'https://raw.githubusercontent.com/smartmaps/regions-cz/master/regions-cz.json',
        style: borderStyles.regions,
      },
      {
        url: 'https://raw.githubusercontent.com/smartmaps/czech-republic/master/czech-republic.json',
        style: borderStyles.country,
      },
    ];
...
const handleMapClick = (e) => {
  ...
  const a =
    ...
    Math.cos((lat * Math.PI) / 180) *
      Math.cos((currentTarget.value.lat * Math.PI) / 180) *
      Math.sin(dLon / 2) *
      Math.sin(dLon / 2);
  ...
  L.value
    .marker([currentTarget.value.lat, currentTarget.value.lng], { icon: dotIcon('#22c55e') })
    .addTo(map.value);
  L.value
    .polyline(
      [
        [lat, lng],
        [currentTarget.value.lat, currentTarget.value.lng],
      ],
      { color: '#3b82f6', weight: 2, dashArray: '8, 8', opacity: 0.6 },
    )
    .addTo(map.value);
};
...
<template>
  <div class="flex h-screen w-screen overflow-hidden bg-slate-50 font-sans text-slate-900">
    <aside class="flex h-full w-80 shrink-0 flex-col border-r border-slate-200 bg-white shadow-sm">
      <div class="border-b border-slate-50 p-6">
        <div class="mb-1 flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 text-xl font-bold uppercase tracking-tighter text-white shadow-lg shadow-blue-100"
          >
            M
          </div>
          <h1 class="text-xl font-bold tracking-tight text-slate-800">Slepá Mapa</h1>
        </div>
        <p class="ml-[3.25rem] text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">
          Česká Republika
        </p>
      </div>
      <nav class="custom-scrollbar flex-1 space-y-1 overflow-y-auto p-4 text-slate-600">
        ...
        <button
          ...
          @click="currentMode = cat.id"
        >
          <span class="text-lg opacity-80">{{ cat.icon }}</span> {{ cat.name }}
        </button>
      </nav>
      <div class="mt-auto border-t border-slate-100 bg-slate-50/50 p-6">
        <div class="mb-4 grid grid-cols-2 gap-3">
          <div class="rounded-2xl border border-slate-100 bg-white p-3 shadow-sm">
            <span class="mb-1 block text-[9px] font-black font-bold uppercase text-slate-400"
              >Kolo</span
            ><span class="text-base font-bold text-slate-700"
              >{{ currentRound }} / {{ totalRoundsInGame }}</span
            >
          </div>
          <div class="rounded-2xl border border-slate-100 bg-white p-3 shadow-sm">
            <span class="mb-1 block text-[9px] font-black font-bold uppercase text-slate-400"
        ...
        <button
          class="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 py-3.5 text-sm font-bold text-white shadow-xl transition-all hover:bg-slate-800 active:scale-[0.98]"
          @click="restartGame"
        >
          🔄 Restartovat hru
        </button>
    ...
    <main class="relative flex h-full min-w-0 flex-1 flex-col bg-[#f8fafc] p-4 md:p-6 lg:p-8">
      <div class="mb-6 flex shrink-0 items-center justify-between px-2 text-slate-800">
        <div v-if="currentTarget" class="flex flex-col gap-1">
          <div class="flex items-center gap-2">
            <span
              class="rounded-md bg-blue-100 px-2 py-0.5 text-[10px] font-bold uppercase text-blue-600"
              >{{ categories.find((c) => c.id === currentMode)?.name }}</span
            ><span class="h-1 w-1 rounded-full bg-slate-300"></span
            ><span class="text-xs font-bold font-medium italic text-slate-400">Najdi na mapě</span>
          </div>
          <h2 class="text-4xl font-black uppercase tracking-tight">
            Najdi: <span class="text-blue-600">{{ currentTarget.name }}</span>
          </h2>
        </div>
      </div>
      <div
        class="relative flex-1 overflow-hidden rounded-[2.5rem] border-[12px] border-white bg-white shadow-2xl shadow-blue-900/5 ring-1 ring-slate-200/50"
      >
        <div id="map" class="h-full w-full"></div>
        <transition name="card-pop">
          <div
            v-if="lastGuess"
            class="absolute bottom-6 left-6 right-6 z-[1000] flex items-center justify-between rounded-3xl border border-white/40 bg-white/80 p-5 shadow-2xl ring-1 ring-black/5 backdrop-blur-xl"
          >
            <div class="max-w-[60%] pl-2">
              <h3 class="mb-1 text-xl font-bold leading-tight text-slate-900">
                {{ currentTarget?.name }}
              </h3>
              <p class="line-clamp-2 text-xs italic leading-relaxed text-slate-600">
                "{{ currentTarget?.info }}"
              </p>
            </div>
            <div class="flex shrink-0 items-center gap-6 border-l border-slate-200/50 pl-6">
              <div class="text-right">
                <p class="mb-1 text-[9px] font-black uppercase tracking-tighter text-slate-400">
                  Získané Body
                </p>
                <div
                  :class="lastGuess.points > 3500 ? 'text-green-600' : 'text-orange-500'"
                  class="text-2xl font-black tabular-nums"
                >
                  +{{ lastGuess.points.toLocaleString() }}
                </div>
                <p class="text-[10px] font-bold text-slate-400">
                  Odchylka: {{ lastGuess.distance }} km
                </p>
              </div>
              <button
                class="rounded-2xl bg-blue-600 px-8 py-4 font-bold text-white shadow-lg shadow-blue-200 transition-all hover:bg-blue-700 active:scale-95"
                @click="
                  currentRound < totalRoundsInGame
        ...
        <div
          ...
          class="absolute inset-0 z-[2000] flex items-center justify-center bg-slate-900/40 px-4 text-center backdrop-blur-md"
        >
          <div
            class="w-full max-w-sm rounded-[3rem] border border-slate-100 bg-white p-12 shadow-2xl"
          >
            <div
              class="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-blue-50 text-3xl"
            >
              🏆
            </div>
            <h2 class="mb-2 text-sm font-bold uppercase tracking-[0.4em] text-blue-600">
              Hra dokončena
            </h2>
            <div class="my-6">
              <p
                class="mb-1 text-[10px] font-black font-bold uppercase tracking-widest text-slate-400"
              >
                Konečné skóre
              </p>
            ...
            <button
              class="w-full rounded-2xl bg-blue-600 py-5 text-xl font-bold text-white shadow-2xl transition hover:bg-blue-700 active:scale-95"
              @click="restartGame"
            >
              Hrát znovu
            </button>
