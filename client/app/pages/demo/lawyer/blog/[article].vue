...
useHead(() => ({ title: `${article.value?.title} — Veritas` }));
</script>
      ...
      <div class="container-x">
        <div class="mx-auto max-w-3xl">
          <div class="reveal flex flex-wrap items-center gap-4 text-sm">
            <span
              class="rounded-full bg-brand px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-white"
            >
              {{ article!.category }}
            </span>
            <span class="flex items-center gap-2 text-brand-muted">
              <span class="material-symbols-outlined text-lg text-brand">calendar_today</span>
              {{ article!.date }}
            </span>
            <span class="flex items-center gap-2 text-brand-muted">
              <span class="material-symbols-outlined text-lg text-brand">person</span>
              {{ article!.author }}
            </span>
          </div>

          <h1 class="reveal mt-6 text-4xl leading-tight sm:text-5xl">
            {{ article!.title }}
          </h1>
        </div>
        ...
        <div class="mt-14 grid gap-8 md:grid-cols-3">
          ...
          >
            ...
            <div class="flex flex-1 flex-col p-6">
              ...
              <h3 class="mt-2 text-lg leading-snug">{{ a.title }}</h3>
            </div>
          </NuxtLink>
        </div>
      </div>
